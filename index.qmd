...
![Figure 3 [@Singh2023]](Features and Target.png)


![Figure 4](boxplot.png)
...
Boxplot without the extreme outlier.
```{r}
...
```
```{r, message=FALSE}
randomForest::varImpPlot(fit_rf$finalModel, scale=FALSE, type=1)
```
...
In closing, the Random Forest algorithm is a powerful supervised learning model that combines multiple uncorrelated decision trees to form a forest. It employs two sampling methods: k-fold cross-validation and bootstrap. In k-fold cross-validation, decision trees are constructed based on feature importance and accuracy decrease, while in bootstrap, training data is randomly sampled with replacement. The algorithm can be applied to both classification and regression tasks, with the prediction determined by the majority vote or average of the leaf nodes, respectively. The Random Forest algorithm offers a versatile approach to solving complex problems and is widely used in various domains of machine learning and data analysis.
