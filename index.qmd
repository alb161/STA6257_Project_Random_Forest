...
3.  Each ach node is split into additional internal nodes using Gini Impurity or Permutation Importance. The Gini Impurity is calculated by summing the Gini decrease across every tree in the forest and divided by the number of trees in the forest to provide an average. The Permutation Importance is calculated by evaluating the decrease in accuracy when a feature is randomly shuffled over all trees in the forest.
...
The extreme outlier for the feature serum cholesterol is removed from the dataset and validated in the box plot.
