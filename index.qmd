...
![Figure 2](Features and Target.png)
...
Comparison of cholestrol variable with and without the extreme outlier.
```{r}
...
The training model was assessed for the best possible number of decision trees, nodes, and number of features to consider at each node. After performing the following procedures, the greatest accuracy was determined to be 200 trees with a node size of 8 and 2 features considered at each node.  
The first tuning parameter evaluated was the number of features the model could consider at each node in a tree.
...
best_mtry
```
The subsequent procedure evaluates the optimal number of nodes for the model.
...
fit_rf <- train(heart.disease~.,
    ...
    maxnodes = 8)
```
The final training model produced an accuracy of 0.8947 compared to the default training model with an accuracy of 0.8472, a 4.75% improvement in accuracy over the default RF parameters.
...
```
The model has an estimated false positive rate of 25%. False positive rate is calculated by the number of false positives divided by the sum of false positives and true negatives. FPR= FP/(FP + TN). 
