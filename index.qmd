...
2.  Specify a node size d from the k features in step 1 using the best split point discussed in step 3.
3.  For bootstrap feature sampling method, each node is split into additional internal nodes using Gini Impurity. The Gini Impurity is calculated by summing the Gini decrease across every tree in the forest and divided by the number of trees in the forest to provide an average. For the repeated k-fold cross-validation feature sampling method, each node is split into additional internal nodes using Permuation Importance. The Permutation Importance is calculated by evaluating the decrease in accuracy when a feature is removed and further divided into classes.
