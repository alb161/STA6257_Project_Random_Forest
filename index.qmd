...
The Random Forest algorithm is a supervised learning model consisting of multiple uncorrelated un-pruned decision trees creating a forest. The algorithm can be used for classification or regression models. The target selected for the algorithm identifies if the algorithm will use a classification or regression model. The algorithm has the capability to sample features using various methods. Two feature sampling methods types were reviewed during the statistical modeling process using the r caret package: k-fold cross-validation and bootstrapping. The k-fold cross-validation method creates subsets of the data and then randomly shuffles the observations into k subset folds where k is arbitrary set. The bootstrap method randomly selects n number of observations with replacement [@Abdulkareem2021].
...
The target variable, heart disease, was set as a factor. The thal feature was transformed for clarity and set as a factor variable.
