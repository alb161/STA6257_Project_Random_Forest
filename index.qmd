...
The basic steps of the Random Forest algorithm using repeated k-fold cross-validation method are outlined below:

1. The data randomly shuffled and split into k groups. The k value is selected by the operator and is arbitrary.
2. One of the k groups is held back for testing while the remaining groups are fit and trained separately.
3. The k group left out is evaluated in testing.
4. The evaluation score is retained and the model is discarded.
5. Step 1 through 4 are repeated for n times where n is another arbitrary value selected by the operator [@Abdulkareem2021].

The basic steps of the Random Forest algorithm using the bootstrap sampling method are as follows:

1. The data split into two-thirds for training and one-third for testing. The data is sampled with replacement with the data chosen referred to the bag, whereas the remaining are called out of bag samples. The out of bag samples at each node is then used to estimate unbiased classification error.
2. Randomly select k features from a total of m features where k < m.
3. Specify a minimum and maximum node size d for the tree nodes.
4. Each node is split into additional internal nodes using the Gini Impurity. The Gini Impurity is calculated by summing the Gini decrease across every tree in the forest and divided by the number of trees in the forest to provide an average. 
5. Repeat the previous steps 1 through 4 steps until l node is remaining.
6. Continue to build the forest by repeating steps 1 through 5 for n number of times to create n number of trees .
7. The test data is evaluaed against the model [@Jaiswal2017].
...
In closing, the Random Forest algorithm is a powerful supervised learning model that combines multiple uncorrelated decision trees to form a forest. Two sampling methods were reviewed: repeated k-fold cross-validation and bootstrap. In repeated k-fold cross-validation, decision trees are constructed based a number of k folds arbitrarily set, while in bootstrap training data is randomly sampled with replacement. The algorithm can be applied to both classification and regression tasks, with the prediction determined by the majority vote or average of the leaf nodes, respectively. The Random Forest algorithm offers a versatile approach to solving complex problems and is widely used in various domains of machine learning and data analysis.
